...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.word_embeddings = nn.Embedding(config.vocab_size, config.hidden_size, padding_idx=0) #A fucntion.(num_embeddings, embedding_dim)
    ...
    def forward(self, input_ids, token_type_ids=None):
        ...
        position_ids = position_ids.unsqueeze(0).expand_as(input_ids) # 维度变为[seq_length,input_ids]
        ...
        embeddings = self.LayerNorm(embeddings) #归一化
...
class BertSelfAttention(nn.Module):
    def __init__(self, config, output_attentions=False, keep_multihead_output=False):
        ...
        self.dense2 = nn.Linear(868, 768)   # 全连接从868维到768维
        self.act_fn = torch.nn.functional.relu   # 激活函数设为relu

    def transpose_for_scores(self, x):
        new_x_shape = x.size()[:-1] + (self.num_attention_heads, self.attention_head_size)
        x = x.view(*new_x_shape)
        return x.permute(0, 2, 1, 3) # permute是位置交换函数，代表原来的（0，1，2，3）排列变成（0，2，1，3）

    def forward(self, hidden_states, attention_mask, hidden_states_ent, head_mask=None):   # 加入参数hidden_states_ent
        ...
        # 拼接hidden_states_ent的信息  [32,384,100]
        mix = torch.cat((mixed_query_layer, hidden_states_ent), 2)   # Q后面接100维, 变成[32,384,868]
        temp = self.dense2(mix)  # 全连接层，从868维变为768
        mixed_query_layer = self.act_fn(temp)  # relu激活
...
class BertAttention(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask, hidden_states_node):    #加入hidden_states_node
        self_output = self.self(input_tensor, attention_mask, hidden_states_node)
...
#-----------------------------新增intermediate_ernie---------------------------
class BertIntermediate_ernie(nn.Module):
    def __init__(self, config):
        super(BertIntermediate_ernie, self).__init__()
    ...
    #def forward(self, hidden_states, hidden_states_ent):     # 去除hidden_states_ent的输入
    def forward(self, hidden_states):
        ...
        #print(hidden_states_ent.size(0))
        #print(hidden_states_ent.size(1))
        #print(hidden_states_ent.size(2))
        #hidden_states_ent_ = self.dense_ent(hidden_states_ent)
        #print(hidden_states_ent_)
        #print(hidden_states_ent_.size(0))
        #print(hidden_states_ent_.size(1))
        #print(hidden_states_ent_.size(2))


        #hidden_states_1 = self.dense_1(hidden_states)
        #hidden_states_ent_1 = self.dense_1_ent(hidden_states_ent)
        #print(hidden_states_)
        #print(hidden_states_.size(0))
       # print(hidden_states_ent_)
       # print(hidden_states_ent_.size(0))
        #hidden_states = self.intermediate_act_fn(hidden_states_+hidden_states_ent_)
        hidden_states = self.intermediate_act_fn(hidden_states_)
...
#------------------------------------新增output--------------------------------------
class BertOutput_ernie(nn.Module):
    def __init__(self, config):
        super(BertOutput_ernie, self).__init__()
        self.dense = nn.Linear(config.intermediate_size, config.hidden_size)
...
class BertLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, hidden_states_node):
...
#----------------------------------新增层----------------------------
class BertLayer_ernie(nn.Module):
    def __init__(self, config):
        super(BertLayer_ernie, self).__init__()
        self.attention = BertAttention(config)
        self.intermediate = BertIntermediate_ernie(config)
        self.output = BertOutput_ernie(config)

    def forward(self, hidden_states, attention_mask, hidden_states_node):
        attention_output = self.attention(hidden_states, attention_mask, hidden_states_node)   # node的输入改到attention层
        attention_output_ent = hidden_states_node
...
class BertEncoder(nn.Module):
    def __init__(self, config, output_attentions=False, keep_multihead_output=False):
        ...
        layer_ernie = BertLayer_ernie(config)
        layers = []
        #for _ in range(6):
            #layers.append(copy.deepcopy(layer))
        for _ in range(12):
            layers.append(copy.deepcopy(layer_ernie)) 
        self.layer = nn.ModuleList(layers) # nn.ModuleList函数的意思是把一整个模型按层分开，变成一个list。
    # ernie 5个输入变为3个 ， bert为2个输入
    def forward(self, hidden_states, attention_mask, hidden_states_node, output_all_encoded_layers=True, head_mask=None):
        all_encoder_layers = []
        all_attentions = [] #？？
        for layer_module in self.layer:
            # 每一层的输出hidden_states也是下一层layer_moudle（BertLayer）的输入，这样就连接起来了各层encoder
            hidden_states, hidden_states_ent = layer_module(hidden_states, attention_mask, hidden_states_node)

            if self.output_attentions:     # ernie没有
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, hidden_states_node=None, output_all_encoded_layers=True, head_mask=None):
...
class BertForQuestionAnswering(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, hidden_states_node=None, start_positions=None,
                end_positions=None, head_mask=None):
        #print(hidden_states_node)
        #print(type(hidden_states_node))
        outputs = self.bert(input_ids, token_type_ids, attention_mask, hidden_states_node,
                                                       output_all_encoded_layers=False,
                                                       head_mask=head_mask)
        if self.output_attentions:
            all_attentions, sequence_output, _ = outputs
